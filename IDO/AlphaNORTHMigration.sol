...
contract aNORTHMigration is Ownable {
    ...
    function migrate() external onlyInitialized() {
        uint256 amount = aNORTH.balanceOf(msg.sender);
        require(amount <= maxAmt,"Exceed the specified amount");
        require(startTimestamp < block.timestamp,"Not started yet");
        require(block.timestamp < endTimestamp, "swapping of aNORTH has ended");

        aNORTH.transferFrom(msg.sender, address(this), amount);
        senderInfo[msg.sender] = senderInfo[msg.sender].add(amount);
        NORTH.transfer(msg.sender, amount);
    }

    function reclaim() external {
        require(senderInfo[msg.sender] > 0, "user has no aNORTH to withdraw");
        require(
            block.timestamp > endTimestamp,
            "aNORTH swap is still ongoing"
        );

        uint256 amount = senderInfo[msg.sender];
        senderInfo[msg.sender] = 0;
        aNORTH.transfer(msg.sender, amount);
    }

    function withdraw() external onlyOwner() {
        require(block.timestamp > endTimestamp, "swapping of aNORTH has not ended");
        uint256 amount = NORTH.balanceOf(address(this));
