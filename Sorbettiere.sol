...
contract Sorbettiere is Ownable {
    ...
    IERC20 immutable public north; // The ICE TOKEN!!
    ...
    constructor(
        IERC20 _north,
        uint256 _northPerSecond,
        uint256 _startTime,
        uint256 _endTime
    ) {
        north = _north;
        
        icePerSecond = _northPerSecond;
        startTime = _startTime;
        endTime = _endTime;
    }
    
    function setStartTime(uint256 _seconds) external onlyOwner {
        startTime = _seconds;
    }

    function setEndTime(uint256 _seconds) external onlyOwner {
        endTime = _seconds;
    }
    ...
    function setIcePerSecond(uint256 _northPerSecond,  bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        icePerSecond= _northPerSecond;
...
// How many pools are in the contract
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                lastRewardTime: uint256(lastRewardTime),
                accIcePerShare: 0
            })
        );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (pool.stakingTokenTotalAmount == 0) {
            pool.lastRewardTime = uint256(block.timestamp);
            return;
        }
    ...
    // Safe north transfer function. Just in case if the pool does not have enough ICE token,
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns(uint256) {
        uint256 iceTokenBalance = north.balanceOf(address(this));
        if (iceTokenBalance == 0) { //save some gas fee
            return _amount;
        }
        if (_amount > iceTokenBalance) { //save some gas fee
            north.safeTransfer(_to, iceTokenBalance);
            return _amount - iceTokenBalance;
        }
        north.safeTransfer(_to, _amount);
        return 0;
    }

    function withdrawNorth(uint256 _val) public onlyOwner {
        north.safeTransfer(address(msg.sender), _val);
    }
}